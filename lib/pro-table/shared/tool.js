...
var normalizeDataSource = function (_a) {
    ...
    var height = dataSource.reduce(function (h, item, index) {
        ...
        leftColumns.forEach(function (col) {
            ...
            var multiple = field.length / (wordLimit * divMultiple);
            if (col.extra && col.extra.merge) {
                var mergeArr = Array.isArray(col.extra.merge)
                    ? col.extra.merge
                    : [col.extra.merge];
                mergeArr.forEach(function (_a) {
                    ...
                    if (mergeField) {
                        var currMultiple = mergeField.length / (limit * getMultiiple(mergeField));
